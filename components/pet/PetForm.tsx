...
export default function PetForm() {
  ...
  const calcCalories = () => {
    ...
    return Math.round(rer * MULTIPLIERS[petType][status])
  }
    ...
    <div className="space-y-6">
        ...
        {(['dog', 'cat'] as const).map(t => (
          <button key={t} onClick={() => setPetType(t)}
            className={`flex-1 py-2.5 rounded-lg border text-sm font-medium transition-all ${
              petType === t ? 'border-green-700 bg-green-50 text-green-700' : 'border-gray-200 text-gray-500'
            }`}>
            {t === 'dog' ? '🐶 Dog' : '🐱 Cat'}
          </button>
        ))}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input value={health} onChange={e => setHealth(e.target.value)} placeholder="e.g. none"
            className="w-full h-10 px-3 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-green-700" />
        </div>
        <div className="col-span-2">
          <label className="block text-xs uppercase tracking-wide text-gray-500 mb-1">Preferred ingredients</label>
          <input value={ingredients} onChange={e => setIngredients(e.target.value)} placeholder="e.g. chicken, rice"
            className="w-full h-10 px-3 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-green-700" />
        </div>
      </div>
      ...
      {kcal && (
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="text-xs text-gray-500 mt-1">kcal / day · {STATUS_LABELS[status]}</div>
          <div className="text-xs text-gray-400 mt-2 pt-2 border-t border-gray-200">
            Range: {Math.round(kcal * 0.5).toLocaleString()}–{Math.round(kcal * 1.5).toLocaleString()} kcal/day
          </div>
        </div>
      )}

      <button onClick={handleSubmit} disabled={!kcal || loading}
      ...
      {plan && (
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            {plan.meals.map((m: any) => (
              <div key={m.time} className="flex gap-3 bg-gray-50 rounded-lg p-3 mb-2">
                <div className="text-xs font-medium text-green-700 w-16 pt-0.5">
                  {m.time}<br/><span className="font-normal text-gray-400">{m.amount}</span>
                </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            {plan.supplement && <p className="text-xs text-gray-500 mt-3"><strong className="text-gray-700">Supplement:</strong> {plan.supplement}</p>}
            {plan.hydration && <p className="text-xs text-gray-500 mt-2"><strong className="text-gray-700">Water:</strong> {plan.hydration}</p>}
          </div>
        </div>
      )}
    </div>
